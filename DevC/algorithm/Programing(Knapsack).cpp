...
int main(){
	...
	int a[]={0,3,4,5,7,6,9};			//weights of items
	...
	for(j=0;j<=b;j++){
		maxv[0][j]=0;					//khong duoc chon do vat nao->max=0
	}
	...
	for(i=1;i<=n;i++){
		for(j=1;j<=b;j++){
			...
			temp=maxv[i-1][j-a[i]]+c[i];	//chon vat i
			if(j>=a[i]&&maxv[i][j]<temp)
				maxv[i][j]=temp;
	...
	while(i>0) {
		if(maxv[i][j]==maxv[i-1][j-a[i]]+c[i]){
			count++;
			p[count]=i;
			j=j-a[i];
		}
